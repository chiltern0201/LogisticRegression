...
df = pd.read_excel("telco_customer_churn.xlsx",sheet_name = "telco_customer_churn", header=0)

# clean up data
df["gender"] = pd.get_dummies(df["gender"])
df["partner"] = pd.get_dummies(df["partner"])
df["payment_mode"] = pd.get_dummies(df["payment_mode"])
df["contract_type"] = pd.get_dummies(df["contract_type"])
df["churn"] = pd.get_dummies(df["churn"])

# formats the input data into two arrays, one of independant variables
# and one of the dependant variable
X = df[["gender","partner","payment_mode","contract_type","tenure","charges_per_month"]]
X = np.array(X)
